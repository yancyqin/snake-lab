...
| `king` | no — defaults to off | `king=1` enables **king-snake mode** for the room. Only honored on first connect (when the room is created). King mode rules — bigger snake wins, smaller snake survives:<br>• **Head-on-head:** bigger eats smaller (absorbs length). Multi-way tie at top length → all die.<br>• **Head-on-body:** if `aggressor.length >= victim.length`, aggressor eats victim. Otherwise the aggressor *passes through* — no death.<br>• **Mutual head-on-body with equal lengths:** both die.<br>• **Self-collision is disabled** — a snake passes through its own body.<br>Regular mode is the classic-snake rules (wall/self/any-body kills you; head-on-head kills both). |
