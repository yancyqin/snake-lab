...
| `king` | no — defaults to off | `king=1` enables **king-snake mode** for the room. Only honored on first connect (when the room is created). When your head hits another snake's body: in regular mode the aggressor dies; in king mode the aggressor lives **only if `aggressor.body.length >= victim.body.length`** — then the defender dies and the aggressor absorbs the defender's length. If the aggressor is shorter, it dies (classic outcome). Head-on-head: both die in either mode. Mutual aggression (A→B and B→A): both die regardless of length. |
